...
public class AdminController {
    ...
    public ResVo hide(@RequestBody HideDto dto){return service.hide2(dto);}
    /*5.게시물 신고 취소
    ...
    @Operation(summary="신고 취소",description="신고받은 글(고기잡담 글, 고기잡담 글의 댓글, 고기집 후기, 정육점 후기 중 신고당한 글) 신고 취소" +
            ...
            "<br>pk: 해당 글 pk")
    public ResVo cancelReport(@RequestBody CancelReportDto dto){return service.cancelReport2(dto);}
    /*6.사용자(USER,OWNER)블랙 리스트(정지)
    1)신고 글 리스트에 오른 글의 작성자
    2)노쇼 카운트 2회 이상인 사용자*/
    @GetMapping("/black")
    @Operation(summary="계정 관리 리스트",description="신고받거나 정지된 USER(이용자),OWNER(가게 주인) 리스트<br>1)신고 글 리스트에 오른 글의 작성자" +
